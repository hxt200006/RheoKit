...
def build_masks(time_ps, tmin_ps=2.0, tmax_ps=None, log=None,*,silent=False):
    ...
    masks = MaskInfo(
        tmin_ps=tmin, tmax_ps=tmax,
        i0_eta=i0_eta, i1_eta=i1_eta, i1_sigma=i1_sigma,
        eta_mask=eta_mask, sigma_mask=sigma_mask,
        n_eta=n_eta, n_sigma=n_sigma,    
    )
    return masks

# Main Logic 
def cutoff_checker(parquet_path, *, candidate_times_ps, W_fraction=0.10,eps_rel=0.02, eps_abs=None, log_name="plateau_check.log"):
    """
    Evaluate whether the viscosity curve is approximately flat over a trailing
    window ending at each candidate cutoff time.
    ...
    if log:
        ...
        log.add(f"eps absolute        : {eps_abs if eps_abs is not None else "No Threshold Set"}")
        log.add()
        log.add()
        if eps_abs is None:
            log.add("Candidate cutoff passes if relative range < eps relative")
        else:
            log.add("Candidate cutoff passes if absolute range < eps absolute")
        log.add()
    ...
    ### Write Table
    W_cut, W_w, W_n, W_med, W_rel, W_abs, W_pass = 12, 12, 9, 10, 12, 12, 6
    ...
    if log:
        ...
        if np.any(floor_used):
            log.add("NOTE:")
            log.add("The trailing window was clamped to the internal minimum (0.5 ps) for one or")
            log.add("more candidates. This typically happens for early cutoffs where the fractional")
            log.add("window would be smaller than 0.5 ps. Consider increasing W_fraction or starting")
    ...
    return CutoffResult(
        ...
        candidate_times_ps=user_times,mapped_indices=mapped_idx,
        pass_flags=pass_flags,rel_range=rel_range,abs_range=abs_range,
        window_sizes_ps=W_used_ps,window_counts=npts_win,
        eta_median=eta_med, table=table
    )
